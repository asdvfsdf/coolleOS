...
//这是通过调用syscall手动引起,或许使用变参列表更合适
//这里不是保存寄存器的值，而是将传入的值存入寄存器中
static inline uint64_t internal_syscall(uint64_t _sysno, uint64_t _a0, uint64_t _a1, uint64_t _a2, uint64_t _a3, uint64_t _a4, uint64_t _a5)
{
	...
	register uint64_t sysno asm("a7") = _sysno;
	asm volatile ("ecall");
	return a0;
}
